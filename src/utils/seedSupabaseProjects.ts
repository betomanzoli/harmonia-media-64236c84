...
export const seedSupabaseWithProjects = async () => {
  try {
    ...
    console.log('Finished seeding Supabase with projects from localStorage');
    return true;
  } catch (error) {
...
export const initializeSupabaseProjects = async () => {
  try {
    ...
    if (hasRun === 'true') {
      console.log('Supabase projects already seeded, skipping');
      return;
    }
    ...
    if (success) {
      ...
      console.log('Failed to seed Supabase with projects, will try again next time');
    }
  } catch (error) {
