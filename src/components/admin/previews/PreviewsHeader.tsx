...
const PreviewsHeader: React.FC<PreviewsHeaderProps> = ({
  scrollToNewForm
}) => {
  ...
  const approvalRate = projects.length > 0 ? Math.round(projects.filter(p => p.status === 'approved').length / projects.length * 100) : 0;
  ...
  return <Card className="border-harmonia-green/30">
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, index) => <div key={index} className={`border rounded-lg p-4 flex items-center space-x-4 ${stat.color}`}>
              ...
              <div>
                ...
                <div className="flex items-center">
                  ...
                  {stat.trend && <span className={`ml-2 text-xs flex items-center ${stat.trend.isPositive ? 'text-green-500' : 'text-red-500'}`}>
                      {stat.trend.icon}
                      {stat.trend.value}%
                    </span>}
                </div>
